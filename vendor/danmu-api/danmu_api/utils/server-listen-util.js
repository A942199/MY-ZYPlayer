...
export async function listenOnAllInterfaces(server, port, options = {}) {
  ...
  const explicitHost = String(process.env.DANMU_API_HOST || '').trim();

  // Embedded MY-ZYPlayer runtime: never expose the local danmu_api to LAN.
  if (explicitHost) {
    const address = await listenOnce(server, {
      port,
      host: explicitHost
    });
    return createBinding(address, explicitHost, port, false);
  }
