...
// The embedded runtime writes configuration under Electron userData instead
// of the read-only packaged resources directory.
const configDir = process.env.DANMU_API_CONFIG_DIR
  ? path.resolve(process.env.DANMU_API_CONFIG_DIR)
...
async function startServer() {
  ...
  if (detectNodeDeployPlatform() === 'node' && process.env.DANMU_API_DISABLE_FAVORITE_SCHEDULER !== '1') {
  ...
  // The embedded runtime uses an ephemeral proxy port to avoid collisions.
  const configuredProxyPort = Number.parseInt(process.env.DANMU_API_PROXY_PORT ?? '', 10);
  const proxyPort = Number.isNaN(configuredProxyPort) ? 5321 : configuredProxyPort;
  proxyServer = createProxyServer();
  const proxyBinding = await listenOnAllInterfaces(proxyServer, proxyPort, {
